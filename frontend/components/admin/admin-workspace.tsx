    ...
    <div className="admin-shell">
      ...
      <div className="admin-content">
        ...
        <main className="admin-main">
          ...
          {section === "import" && <><PageHeading number="04" eyebrow="CATALOGUE IMPORT" title={<>A fresh source<br /><em>of knowledge.</em></>} description="Replace the live scenario catalogue in one atomic update. Your routing prompts and model settings are preserved." /><form className="admin-import" onSubmit={(event) => { event.preventDefault(); if (!window.confirm("Replace the current catalogue and reference data? Existing records will be replaced.")) return; const element = event.currentTarget; const form = new FormData(element); void run(async () => { await adminRequest<{ count: number }>("catalog/import-files", { method: "POST", body: form }, token); await load(token); element.reset(); }, "Catalogue imported successfully."); }}><div className="admin-import-icon"><UploadCloud size={34} strokeWidth={1.3} /></div><h2>Import catalogue files</h2><p>JSON · 2 MB per file. For the official Saqta starter kit, supply all five files with the same version. Custom/demo catalogues may omit the optional files.</p><label className="admin-upload" htmlFor="scenarios-file">Scenarios JSON <span>Required</span><input id="scenarios-file" name="scenarios" type="file" accept=".json,application/json" required /></label><label className="admin-upload" htmlFor="knowledge-file">Knowledge base JSON <span>Optional</span><input id="knowledge-file" name="knowledge_base" type="file" accept=".json,application/json" /></label><label className="admin-upload" htmlFor="backend-file">Mock backend JSON <span>Optional</span><input id="backend-file" name="mock_backend" type="file" accept=".json,application/json" /></label><label className="admin-upload" htmlFor="slots-file">Slots JSON <span>Required for official kit</span><input id="slots-file" name="slots" type="file" accept=".json,application/json" /></label><label className="admin-upload" htmlFor="actions-file">Actions JSON <span>Required for official kit</span><input id="actions-file" name="actions" type="file" accept=".json,application/json" /></label><button className="admin-primary" type="submit" disabled={busy}>Replace live catalogue <ArrowRight size={17} /></button></form><p className="admin-import-note">This replaces all existing scenarios. Review your files before importing.</p></>}
        </main>
      </div>
    </div>
  );
