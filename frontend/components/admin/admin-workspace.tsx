    ...
    <div className="admin-shell">
      ...
      <div className="admin-content">
        ...
        <main className="admin-main">
          ...
          {section === "settings" && <><PageHeading number="02" eyebrow="LIVE CONFIGURATION" title={<>The voice behind<br /><em>every answer.</em></>} description="Adjust routing and workflow policies. The active dataset is loaded from PostgreSQL settings on every turn." />{settings && <form className="admin-form" onSubmit={(event) => { event.preventDefault(); void run(async () => setSettings(await adminRequest<Settings>("settings", { method: "PATCH", body: JSON.stringify(settings) }, token)), "Configuration saved. Changes are live for the next turn."); }}><div className="admin-form-row"><label htmlFor="model">Routing model<span>Model ID used to select a scenario</span></label><input id="model" className="admin-input" value={settings.model} onChange={(event) => setSettings({ ...settings, model: event.target.value })} required /></div><div className="admin-form-row"><label htmlFor="threshold">Confidence threshold<span>When confidence falls below this value, Butaq asks for clarification</span></label><div className="admin-threshold"><input id="threshold" type="range" min="0" max="1" step="0.01" value={settings.confidence_threshold} onChange={(event) => setSettings({ ...settings, confidence_threshold: event.target.value })} /><input aria-label="Confidence threshold value" className="admin-input" type="number" min="0" max="1" step="0.01" value={settings.confidence_threshold} onChange={(event) => setSettings({ ...settings, confidence_threshold: event.target.value })} required /></div></div><div className="admin-form-row"><label htmlFor="workflow-enabled">Scenario workflow<span>Collect configured slots and run safe action simulations</span></label><select id="workflow-enabled" className="admin-input" value={settings.workflow_enabled} onChange={(event) => setSettings({ ...settings, workflow_enabled: event.target.value })}><option value="true">Enabled</option><option value="false">Routing only</option></select></div><div className="admin-form-row"><label htmlFor="uncertain-turns">Uncertain turns before handoff<span>Missing workflow slots do not count toward this limit</span></label><input id="uncertain-turns" className="admin-input" type="number" min="1" max="10" value={settings.max_uncertain_turns} onChange={(event) => setSettings({ ...settings, max_uncertain_turns: event.target.value })} required /></div><div className="admin-form-row"><label htmlFor="simulation-mode">Action mode<span>Locked to side-effect-free simulation for this demo</span></label><input id="simulation-mode" className="admin-input" value={settings.simulation_mode} readOnly /></div><div className="admin-form-row"><label htmlFor="routing-prompt">Routing instructions<span>How the assistant selects, clarifies, or escalates a scenario</span></label><textarea id="routing-prompt" className="admin-input" rows={12} value={settings.routing_prompt} onChange={(event) => setSettings({ ...settings, routing_prompt: event.target.value })} required /></div><div className="admin-form-row"><label htmlFor="answer-prompt">Response instructions<span>How the assistant speaks once a scenario is selected</span></label><textarea id="answer-prompt" className="admin-input" rows={12} value={settings.answer_prompt} onChange={(event) => setSettings({ ...settings, answer_prompt: event.target.value })} required /></div><div className="admin-form-actions"><button type="button" className="admin-secondary" disabled={busy} onClick={() => void run(async () => setSettings(await adminRequest<Settings>("settings", {}, token)), "Latest configuration loaded.")}><RefreshCw size={16} /> Reload</button><button className="admin-primary" disabled={busy} type="submit">Save changes <ArrowRight size={17} /></button></div></form>}</>}
          ...
          {section === "import" && <><PageHeading number="04" eyebrow="CATALOGUE IMPORT" title={<>A fresh source<br /><em>of knowledge.</em></>} description="Replace the live scenario catalogue in one atomic update. Your routing prompts and model settings are preserved." /><form className="admin-import" onSubmit={(event) => { event.preventDefault(); const element = event.currentTarget; const form = new FormData(element); void run(async () => { await adminRequest<{ count: number }>("catalog/import-files", { method: "POST", body: form }, token); await load(token); element.reset(); }, "Catalogue imported successfully."); }}><div className="admin-import-icon"><UploadCloud size={34} strokeWidth={1.3} /></div><h2>Import catalogue files</h2><p>JSON files only · Maximum 2 MB per file. Scenarios are required; slots and actions make the catalogue executable.</p><label className="admin-upload" htmlFor="scenarios-file">Scenarios JSON <span>Required</span><input id="scenarios-file" name="scenarios" type="file" accept=".json,application/json" required /></label><label className="admin-upload" htmlFor="slots-file">Slots JSON <span>For workflow</span><input id="slots-file" name="slots" type="file" accept=".json,application/json" /></label><label className="admin-upload" htmlFor="actions-file">Actions JSON <span>For simulation</span><input id="actions-file" name="actions" type="file" accept=".json,application/json" /></label><label className="admin-upload" htmlFor="knowledge-file">Knowledge base JSON <span>Optional</span><input id="knowledge-file" name="knowledge_base" type="file" accept=".json,application/json" /></label><label className="admin-upload" htmlFor="backend-file">Mock backend JSON <span>Optional</span><input id="backend-file" name="mock_backend" type="file" accept=".json,application/json" /></label><label className="admin-upload" htmlFor="utterances-file">Dev utterances JSON <span>Evaluation</span><input id="utterances-file" name="dev_utterances" type="file" accept=".json,application/json" /></label><label className="admin-upload" htmlFor="dialogs-file">Dialog samples JSON <span>Evaluation</span><input id="dialogs-file" name="dialogs_sample" type="file" accept=".json,application/json" /></label><button className="admin-primary" type="submit" disabled={busy}>Replace live catalogue <ArrowRight size={17} /></button></form><p className="admin-import-note">This atomically replaces scenarios, workflow definitions, grounding data, and supplied evaluation fixtures. Review your files before importing.</p></>}
        </main>
      </div>
    </div>
  );
