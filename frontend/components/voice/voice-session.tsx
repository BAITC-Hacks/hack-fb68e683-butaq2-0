      ...
      {current && (
        <div className="mt-8 grid gap-4 text-left md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-white/15 bg-black/80 p-5 text-sm" aria-label="Routing trace">
            ...
            {current.workflow_status !== "idle" ? <div className="mt-4 border-t border-white/15 pt-4"><h3 className="text-xs uppercase tracking-widest text-white/50">Scenario workflow · {current.workflow_status.replaceAll("_", " ")}</h3>{Object.keys(current.collected_slots).length > 0 ? <p className="mt-2 text-xs text-white/70">Collected: {Object.entries(current.collected_slots).map(([key, value]) => `${key}=${Array.isArray(value) ? value.join(", ") : String(value)}`).join(" · ")}</p> : null}{current.missing_slots.length > 0 ? <p className="mt-2 text-xs text-amber-200">Next required: {current.missing_slots[0]}</p> : null}{current.confirmation_required ? <p className="mt-2 text-xs text-amber-200">Explicit confirmation required before simulation.</p> : null}{current.action_trace.length > 0 ? <div className="mt-3 space-y-2">{current.action_trace.map((action, index) => <div className="rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-xs" key={`${action.name}-${index}`}><span className="font-medium text-violet-200">{action.name}</span><span className="ml-2 text-white/50">{action.mode} · {action.status}</span>{action.error ? <p className="mt-1 text-red-200">{String(action.error.message ?? action.error.code ?? "Action error")}</p> : null}</div>)}</div> : null}</div> : null}
