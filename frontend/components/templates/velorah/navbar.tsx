...
export function Navbar({ actionLabel = "Talk to Butaq" }: { actionLabel?: string }) {
  ...
  return (
    <nav aria-label="Main navigation" className="relative z-10 mx-auto flex min-h-24 max-w-7xl flex-wrap items-center justify-between gap-x-2 gap-y-3 px-6 py-5 sm:gap-x-4 sm:px-8">
      ...
      <div className="flex items-center gap-1 text-sm text-white sm:gap-3">
        {NAV_LINKS.map((link) => (
          <a
            ...
            className="rounded-full px-2 py-3 text-white/80 transition-colors hover:bg-white/5 hover:text-white aria-[current=page]:bg-white/10 aria-[current=page]:text-white motion-reduce:transition-none sm:px-4"
          >
            {link.label}
          </a>
        ))}
